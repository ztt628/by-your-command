javascript: (function () {
  ...
  /*let epoch = 1553295600000; let date = new Date(); let week = "" + Math.floor((date.getTime() - epoch) / 604800000);*/ function reqListener() {
    ...
    if (done) {
      ...
      oReq.open(
        "GET",
        "https://ztt628.github.io/by-your-command/" + filenames[game][1] + ".js"
      );
      oReq.send();
    } else {
      alert("Could not read script");
  ...
  if (!done || /*lastWeek !== week ||*/ urgent) {
    ...
    oReq.open(
      "GET",
      "https://ztt628.github.io/by-your-command/" + filenames[game][0] + ".js"
    );
    oReq.send();
  } else {
    try {
